...
#'
#' Evaluates the reliability of copy number variant calls based on coverage, ratio,
#' and genomic region criteria.
#'
...
score_cnv_confidence <- function(cnv_calls, bed_file, current_cor, num_refs,
    ...
    # --- FIX: Vectorised construction of Gene column ---
    ...
    # Remove unused Start.b/End.b columns (they were not used downstream)
...
#'
#' Processes BAM count coverage sets to detect copy number variants (CNVs) per sample.
#'
#' @param rdata_file Character string. Output from \code{\link{run_bam_coverage}}.
#' @param output_file Character string. TSV output for CNV calls. Default \code{"./CNV_calls.tsv"}.
...
run_cnv_calling <- function(rdata_file,
    ...
    for (sample in sample_names) {
        ...
        if (!is.null(raw_calls) && nrow(raw_calls) > 0) {
            ...
            # --- FIX: Genomic.ID created after renaming ---
    ...
    if (save_ed_objects) {
        save(counts, cnv_calls, refs, models, bed_file, ed_objects, file = out_rdata)
    } else {
        save(counts, cnv_calls, refs, models, bed_file, file = out_rdata)
    }
