...
#' @export
run_qc_metrics <- function(rdata_file, min_corr = 0.98, min_cov = 100,
                           min_total_reads = 300000, max_exon_cv = 0.5,
    ...
    if (length(low_corr))
        add_metric(sample_names[low_corr], rep("All", length(low_corr)),
                   "Whole sample", paste("Low correlation:", round(max_corr[low_corr], 2)),
                   rep("All", length(low_corr)))

    low_med <- which(sample_median < min_cov)
    if (length(low_med))
        add_metric(sample_names[low_med], rep("All", length(low_med)),
                   "Whole sample", paste("Low median depth:", round(sample_median[low_med], 2)),
    ...
    if (length(low_reads))
        add_metric(sample_names[low_reads], rep("All", length(low_reads)),
                   "Whole sample", paste("Low total reads:", round(total_reads[low_reads], 0)),
                   rep("All", length(low_reads)))

    exon_median <- apply(dt_counts, 1, median, na.rm = TRUE)
    fail_exon   <- which(exon_median < min_cov)
    if (length(fail_exon)) {
        exon_labels <- paste0(bed_file$gene[fail_exon], ":", exon_in_gene[fail_exon])
        add_metric(rep("All", length(fail_exon)), exon_labels,
                   "Whole exon", paste("Low median depth:", round(exon_median[fail_exon], 2)),
                   bed_file$gene[fail_exon])
    }
    ...
    if (length(high_cv)) {
        exon_labels <- paste0(bed_file$gene[high_cv], ":", exon_in_gene[high_cv])
        add_metric(rep("All", length(high_cv)), exon_labels,
                   "Exon variability",
