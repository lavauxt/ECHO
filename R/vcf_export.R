...
#' @export
export_cnvs_to_vcf <- function(cnv_calls, output_vcf, sample_name = NULL, source = "ECHO", reference = "hg19") {
    ...
    header <- c(
        "##fileformat=VCFv4.2", paste0("##fileDate=", format(Sys.Date(), "%Y%m%d")), paste0("##source=", source),
        paste0("##reference=", reference), "##INFO=<ID=SVTYPE,Number=1,Type=String,Description=\"Type of structural variant (DUP/DEL)\">",
    ...
    records <- apply(key_df, 1, function(key_row) {
        ...
        row_hits <- cnv_calls[as.character(cnv_calls$Chromosome) == chrom & as.integer(cnv_calls$Start) == start & as.integer(cnv_calls$End) == end & tolower(as.character(cnv_calls$Type)) == cnv_type, , drop = FALSE]
        representative <- row_hits[1, , drop = FALSE]
        id <- if ("CNV.ID" %in% colnames(representative)) paste0("CNV_", representative$`CNV.ID`) else paste0(gsub("^chr", "", chrom), "_", start, "_", end, "_", sv_type)
        info_items <- c(paste0("SVTYPE=", sv_type), paste0("SVLEN=", sv_len), paste0("END=", end))
        ...
        if ("Comparator.name" %in% colnames(representative)) {
            ...
            if (refs_val != "") info_items <- c(info_items, paste0("REFS=", refs_val))
        }
        ...
        if ("Reads.ratio" %in% colnames(representative)) {
            ...
            if (!is.na(ratio) && ratio != "NA") info_items <- c(info_items, paste0("READS_RATIO=", round(as.numeric(ratio), 2)))
        }
